...
export default async function BoardPage({ params, searchParams }: BoardPageProps) {
      ...
      return (
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="bg-[#1a1a1a] rounded-lg p-8 text-center">
            <h1 className="text-2xl font-bold text-[#e0e0e0] mb-4">데이터베이스 연결 필요</h1>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      {/* Board Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#e0e0e0] mb-2">{board.nameKo}</h1>
        {board.description && (
          <p className="text-sm text-[#a0a0a0]">{board.description}</p>
        )}
        <div className="mt-2 text-sm text-[#888]">
          전체 게시글 <span className="text-[#e0e0e0] font-medium">{pagination.total}</span>개
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-col lg:flex-row lg:items-center gap-4">
        ...
        {session && (
          <Link
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-[#3b82f6] hover:bg-[#2563eb] text-white rounded-lg transition-colors font-medium min-h-[44px]"
          >
      ...
      <div className="bg-[#1a1a1a] rounded-lg overflow-hidden">
        ...
        <div className="hidden lg:grid lg:grid-cols-[auto_1fr_150px_100px_80px_80px] gap-4 items-center px-4 py-3 border-b border-[#333] bg-[#242424]">
          <div className="w-6"></div>
          <div className="text-sm font-medium text-[#a0a0a0]">제목</div>
          <div className="text-sm font-medium text-[#a0a0a0]">작성자</div>
          <div className="text-sm font-medium text-[#a0a0a0]">날짜</div>
          <div className="text-sm font-medium text-[#a0a0a0]">조회</div>
          <div className="text-sm font-medium text-[#a0a0a0]">좋아요</div>
        </div>
        ...
        {posts.length === 0 ? (
          ...
          <div className="lg:divide-y lg:divide-[#333] flex flex-col gap-3 lg:gap-0 p-3 lg:p-0">
            {posts.map((post: any) => (
              <PostRow
                key={post.id}
                postId={post.id}
