...
export function CooldownRewardWidget() {
  ...
  useEffect(() => {
    const savedStats = localStorage.getItem('openpoker_harvest_stats');
    if (savedStats) {
      ...
      if (parsed.date !== today) {
        localStorage.setItem('openpoker_harvest_stats', JSON.stringify({
          date: today,
          lastReward: 0,
          harvestCount: 0,
          totalToday: 0,
  ...
  useEffect(() => {
    const updateCooldown = () => {
      const lastClaimStr = localStorage.getItem('openpoker_last_harvest');
      if (!lastClaimStr) {
        setIsAvailable(true);
        setTimeRemaining('');
        return;
    ...
    try {
      ...
      if (result.success && result.pointsEarned) {
        ...
        if (result.nextClaimAt) {
          localStorage.setItem('openpoker_last_harvest', result.nextClaimAt);
        }
  ...
  return (
    <div className="relative w-full bg-op-surface border border-op-border rounded-lg p-4 overflow-hidden">
      {/* Background decoration */}
      <div className="absolute inset-0 bg-gradient-to-br from-op-gold/5 via-transparent to-transparent pointer-events-none" />

      {/* Celebration overlay */}
      {showCelebration && (
        <div className="absolute inset-0 bg-gradient-to-t from-op-gold/20 via-op-gold/10 to-transparent animate-pulse pointer-events-none z-10 flex items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            <Sparkles className="w-12 h-12 text-op-gold animate-bounce" />
            <span className="text-2xl font-bold text-op-gold drop-shadow-lg">
              +{stats.lastReward}P
            </span>
          </div>
        </div>
      ...
      <button
        ...
        className={cn(
          ...
          isAvailable
            ? 'bg-gradient-to-br from-op-gold to-op-gold-hover border-op-gold-hover text-op-surface hover:shadow-lg hover:shadow-op-gold/50 hover:scale-105 animate-pulse'
            : 'bg-op-elevated border-op-border text-op-text-muted cursor-not-allowed grayscale'
        )}
      >
      ...
      {stats.harvestCount > 0 && (
        <div className="mt-3">
          <div className="h-1 bg-op-elevated rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-op-gold to-op-gold-hover transition-all duration-500"
              style={{ width: `${Math.min((stats.harvestCount / 5) * 100, 100)}%` }}
            />
          </div>
          <p className="text-xs text-op-text-muted mt-1 text-center">
