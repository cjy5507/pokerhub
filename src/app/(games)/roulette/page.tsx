...
export default function RoulettePage() {
  ...
  const getResultIcon = (multiplier: string) => {
    ...
    return <Crown className="w-8 h-8 text-op-gold" />;
  };
  ...
  const getResultColor = (multiplier: string) => {
    ...
    return 'from-amber-900/50 to-amber-800/50 border-op-gold';
  };
    ...
    <div className="min-h-screen bg-op-bg text-op-text pb-20 lg:pb-0 px-4 py-6">
      ...
      <div className="max-w-5xl mx-auto mb-8">
        <div className="flex items-center justify-center gap-3 mb-4">
          <Disc className="w-8 h-8 text-op-gold" />
          <h1 className="text-3xl font-bold">포인트 룰렛</h1>
        </div>

        {/* Current Points */}
        <div className="bg-op-surface border border-op-border rounded-xl p-4 text-center">
          <div className="text-sm text-op-text-secondary mb-1">보유 포인트</div>
          <div className="text-3xl font-bold text-op-gold">{isLoadingPoints ? '...' : `${currentPoints.toLocaleString()}P`}</div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-4 lg:gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-op-surface border border-op-border rounded-xl p-8">
            <div className="relative mx-auto" style={{ width: 'min(320px, calc(100vw - 64px))', height: 'min(320px, calc(100vw - 64px))' }}>
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 z-20">
                <div
                  className="w-0 h-0 border-l-[20px] border-l-transparent border-r-[20px] border-r-transparent border-t-[40px] border-t-op-gold"
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                >
                  ...
                  {/* Center Circle */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-16 h-16 rounded-full bg-op-surface border-4 border-white shadow-lg" />
                  </div>
          ...
          {/* Bet Selection */}
          <div className="bg-op-surface border border-op-border rounded-xl p-6">
            <div className="text-sm text-op-text-secondary mb-3">배팅 금액 선택</div>
              ...
              {BET_AMOUNTS.map((amount) => (
                <button
                  ...
                  className={cn(
                    ...
                    betAmount === amount
                      ? "bg-op-gold text-black shadow-[0_0_20px_rgba(201,162,39,0.4)]"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-op-surface border border-op-border rounded-xl p-6">
            <div className="flex items-center gap-2 mb-4">
              <BarChart3 className="w-5 h-5 text-op-gold" />
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-op-elevated rounded-lg p-4">
                <div className="text-sm text-op-text-secondary mb-1 flex items-center gap-1">
                  {profit >= 0 ? (
          ...
          <div className="bg-op-surface border border-op-border rounded-xl p-6">
            <div className="flex items-center gap-2 mb-4">
              <History className="w-5 h-5 text-op-gold" />
              <h2 className="text-lg font-bold">최근 기록</h2>
            </div>

            {history.length === 0 ? (
              <div className="text-center py-8 text-op-text-secondary">
                아직 스핀 기록이 없습니다
              </div>
            ) : (
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {history.map((spin, index) => (
                  <div
                    key={index}
                    className="bg-op-elevated rounded-lg p-3 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      <div className="text-xs text-op-text-secondary">{spin.time}</div>
                      <div className="text-sm">
                        <span className="text-op-text-secondary">{spin.bet}P</span>
                        <span className="mx-2 text-op-text-muted">x</span>
