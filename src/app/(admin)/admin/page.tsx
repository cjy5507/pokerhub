...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: '활성 유저 (7일)', value: data.activeUsers.toLocaleString(), icon: Activity, color: 'text-[#c9a227]' },
    { label: '총 게시글', value: data.totalPosts.toLocaleString(), icon: FileText, color: 'text-purple-400' },
  ];

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-[#c9a227]">대시보드</h2>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-xl bg-[#1e1e1e] border border-[#333] p-4">
            ...
            <tbody>
              {data.recentUsers.map((user: { id: string; nickname: string; email: string; role: string; level: number; points: number; createdAt: Date; status: string }) => (
                <tr key={user.id} className="border-b border-[#333]/50 hover:bg-[#333]/30">
                  ...
                  <td className="py-2.5 pr-4 text-[#888]">{user.email}</td>
                  ...
                  <td className="py-2.5 pr-4 text-[#c9a227]">{user.points.toLocaleString()}</td>
                  <td className="py-2.5 text-[#888]">
              ...
              {data.recentUsers.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-[#888]">유저가 없습니다</td>
                </tr>
              )}
            </tbody>
          </table>
